...
- ~~Offer stack + bonus covers~~ DONE. `public/system/*.webp` feeds the hero offer card and **all four** value-stack covers; the last one (10-Minute Joint Mobility & Pain Relief Playbook) landed 17 Sep. No `MediaPlaceholder` remains in the toolkit beat.
...
- LAUNCH pass run 15 Sep 2026 with payments excluded, then a second pass the same day that built payments on Instamojo, reversed to **Razorpay** on 17 Sep. Checkout, thank-you, the three legal pages, the footer, Meta CAPI, GA4, Clarity, Pabbly, the payment layer and `.env.example` are this client's. No Kaizen string, colour or value survives outside `app/_landing/**`.
...
- **Razorpay restoration pass, 17 Sep 2026.** The gateway swap was reversed. Out: `lib/instamojo.ts`, `lib/payment-context.ts`, `app/api/instamojo/{create-payment,webhook,return}`, the `instamojo` config block, the redirect flow on the checkout, `INSTAMOJO-API.md` and every `INSTAMOJO_*` var. Back in: `lib/order-notes.ts` (the five-readable-plus-ten-chunk packer), `app/api/razorpay/create-order`, `app/api/razorpay/webhook`, the `razorpay` config block, and the on-demand SDK loader plus payment sheet on `app/checkout/page.tsx`. Still no npm dependency: the REST API is called over fetch. The context carrier is the order notes again rather than an encrypted token on a URL, so nothing is sealed, nothing depends on an undocumented URL length, and the five human keys (kind, lead_id, name, email, phone) are readable in the dashboard beside the payment. `amountPaise` is derived in `lib/checkout-config.ts` from `PRICE_RUPEES` rather than re-added to `offer.ts`, which is SHAPE's file. Receipt prefix `dpp_`. No `image` on the sheet, because the brand file does not exist. Two rules that came back with the gateway: the webhook must be registered in the dashboard, and `RAZORPAY_WEBHOOK_SECRET` is a separate value from the API keys.
- **Instamojo pass, 15 Sep 2026 (now reversed, kept for the trail).** Razorpay was removed wholesale (both routes, the config block, the SDK loader, the client handler, `lib/order-notes.ts`) and replaced with `lib/instamojo.ts`, `lib/payment-context.ts` and three routes under `app/api/instamojo/`: `create-payment`, `webhook`, `return`. No npm dependency: the REST API is called over fetch, as before. The gateway is a REDIRECT, so the checkout navigates the tab to Instamojo's `longurl` and the buyer returns through `/api/instamojo/return`, which confirms the payment server-side before forwarding to `/thank-you?p=`. Purchase, the server GA4 copy and the Pabbly hand-off all still come from the webhook and nowhere else. Instamojo has no notes field, so the buyer context (fbp, fbc, IP, user agent, GA4 client id, city, occupation, campaign) is sealed with AES-256-GCM and carried on the webhook URL set per payment request. Unconfigured, so the pay button still reports "Payments are not switched on yet. Nothing has been charged."
